...
# the OpenSceneGraph, PLIB, SimGear libraries and FlightGear Flight Simulator.
...
fgbranch="version/2020.3.5" # Branch for FlightHear
sgbranch="version/2020.3.5" # Branch for SimGear
osgbranch="OpenSceneGraph-3.6.5" # Branch for OpenSceneGraph
...
# Adds option to let user download and build the next/testing version of FlightGear
if [ "$*" = "--next" ]; then
	fgbranch="next"
	sgbranch="next"
fi
...
echo "#====== you can run the script \`./fgdata_downloader\` in a different terminal window."
echo "#====== While you run \`flightgear_downloader\` and \`flightgear_compiler\`"
...
echo "======= Selected FlighthGear version is $fgbranch"
echo ""
...
echo ""
echo "#====== Downloading Source Codes Started."
